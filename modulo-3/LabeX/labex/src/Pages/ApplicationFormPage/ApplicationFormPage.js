...
const applyToTrip = () =>{
  ...
  axios
  .post(`${BASE_URL}/trips/${idValor}`,body)
  .then((res)=>{
  ...
  const tripsList = TripsValor ? TripsValor.map((trips) => {
    return ( 
        <option key={trips.id} onClick={() =>setIdValor(trips.id)}>{trips.name}</option>
...
console.log(applyToTrip)
  return (
    <Main>
      ...
      <button onClick={()=>applyToTrip()}> Inscreva-se</button>
    </Main>
  );
};
